...
def prep_data(n=1e5, num_spkrs=2, save_wav=False):
    ...
    all_speakers = [
        glob.glob(os.path.join(spkr_path, "**", "*.flac"), recursive=True)
        for spkr_path in all_speakers
    ]
    ...
    while i <= n:
        ...
        s_slct_LIBRISPEECH = [
            random.sample(all_speakers, NUM_SPEAKERS - 1)
            for x in range(int(a * BATCH_SIZE))
        ]

        # randomly select some speakers batch_size - [a*batch_size] times
        s_slct_flipkart = [
            random.sample(flipkart_speakers, NUM_SPEAKERS - 1)
            for _ in range(BATCH_SIZE - int(a * BATCH_SIZE))
        ]
...
def mix(speakers_list, noise_smpl, sample_num, outDir, save_wav=False):
    ...
    s_rest_audio = [
        librosa.effects.trim(spkr_audio, top_db=20)[0] for spkr_audio in s_rest_audio
    ]
    ...
    for s_audio in s_rest_audio:
        mixed += s_audio
    mixed += noise_audio

    # norm
    norm = np.max(np.abs(mixed))
    target_dvec, target_audio, mixed = (
        target_dvec / norm,
        target_audio / norm,
        mixed / norm,
    )
    ...
    if save_wav:
        ...
        librosa.output.write_wav(
            os.path.join(outpath, "dvec.wav"), target_dvec, sr=SAMPLING_RATE
        )
...
if __name__ == "__main__":
    ...
    # 2)NOISY
    # download and unzip
    Noisy_Dir = os.path.join(DATA_DIR_RAW, "Noisy")
    subdirs = ["airport", "babble", "restaurant"]
    for sub in subdirs:
        # destination folder
    ...
    # Flipkart_dir = os.path.join(Consts.DATA_DIR_RAW, "Flipkart")
    # Flipkart_src = "..."
