...
def train(dataloader, device, lr, num_epochs):
    ...
    if len( chkpt_list ):
        chkpt = chkpt_list[-1]
        extractor.load_state_dict(torch.load(chkpt), map_to_device=device)
    #
    extractor.train()
    ...
    step_size = np.max((dataset_size / 100, 1))
    loss_list = list() # list to hold losses after each step_size number of batches
    #training loop
    for n in tqdm(range(num_epochs), desc="Epoch"):
        for batch_id, batch in tqdm(enumerate(dataloader), desc="Batch"):
            ...
            if batch_id % step_size :
                loss_list.append(loss.item())
                print(f"loss: {loss.item()}")

                if batch_id % (step_size * 10):
                    #save checkpoint
